...
class NPP:
    def __init__(self, instructions, special_library={}):
        ...
        # CONFIG
        self.version = "1.1.0" # placeholder
...
#        print(expression)
    ...
    def process_vars(self):
        ...
        """
#        print(list(self.variables.keys()))
#        print()
#        if self.in_class[1]:
#            print(list(self.classes[self.in_class[0]]["variables"].keys()))
#            print()
#        input(">>> ")
    ...
    def ultimate_split(self, line, split_tokens, group_pairs=(("'", '"'), ('"', "'")), nest_pairs=(("(", ")"), ("[", "]"), ("{", "}")), join_capture=True):
        ...
        if current:
            result.append(current.strip())
        return result
    def special_split(self, line, split_str, in_char1, in_char2, limit=9999, ret_capture_group=False):
        """
...
#                break
            ...
            if not inside_char and line[i:i+len(split_str)] == split_str:
                new_line += "¤" if not ret_capture_group else f"¤{split_str}¤"# placeholder
                i += len(split_str)
                cnt += 1
                continue
    ...
    def execute_functions(self, instruction):
        ...
        if not instruction:
            ...
            condition = self.run_condition(condition)
            while condition:
                code = self.prep_exec(block)
                self.exec_block(code, count)
        ...
        # for loops (for each and loops)
            ...
            arg = self.special_split(instruction[4:], " in ", ('"', "'", "(", "[", "{"), ('"', "'", ")", "]", "}"), 1)
...
#        if self.in_class[1]:
#            print(self.classes[self.in_class[0]]["variables"], left, right)
        istrue = [
